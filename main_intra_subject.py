...
acc_test_list = []
for current_subject in args.subject_list:

    print("\n"+"="*35+f" Start training the dedicated model for {current_subject}-th subject "+"="*35)

    # Dataset setup
    train_loader, valid_loader, test_loader = train_test_split_DataLoader(\
                                            batch_size=args.batch_size, subject_list=[current_subject], exercise_list= args.exercise_list, \
    ...
    if args.en_train:
        train_process(args.num_epoch,model,model_PATH,train_loader,valid_loader,device,optimizer,criterion, scheduler=scheduler, pretrain_model_PATH=args.pretrain_model_PATH)
